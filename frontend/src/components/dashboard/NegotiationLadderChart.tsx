      ...
      {ladder.length === 0 ? (
        ...
        <ResponsiveContainer width="100%" height={260}>
          <BarChart data={ladder} layout="vertical" margin={{ left: 8, right: 16 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#1f2c4a" horizontal={false} />
            <XAxis type="number" stroke="#8a9ab2" fontSize={11} tickFormatter={(v) => `$${v}`} />
            <YAxis type="category" dataKey="step" stroke="#8a9ab2" fontSize={11} width={72} />
            <Tooltip contentStyle={tooltipStyle} formatter={(v: number) => fmtMoney(v)} />
            <Bar dataKey="amount" radius={[0, 4, 4, 0]}>
              {ladder.map((d, i) => (
                <Cell key={i} fill={d.kind === "carrier" ? "#ffae42" : "#4ea1ff"} />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
...
export function NegotiationLadderInline({ call }: { call: CallRecord }) {
  ...
  return (
    <section style={{ marginBottom: 24 }}>
      <div style={{
        fontSize: 11, fontWeight: 700, color: "#8a9ab2",
        textTransform: "uppercase", letterSpacing: 0.8, marginBottom: 10,
      }}>
        Negotiation ladder
      </div>
      <div style={{
        background: "#111a2e", border: "1px solid #1f2c4a", borderRadius: 10, padding: 14,
      }}>
        {call.loadboard_rate > 0 && (
          <LadderRow label="Posted" amount={call.loadboard_rate} max={max} color="#8a9ab2" />
        )}
        {ladder.map((s) => (
          <LadderRow key={s.step} label={s.step} amount={s.amount} max={max}
            color={s.kind === "carrier" ? "#ffae42" : "#4ea1ff"} />
        ))}
      </div>
    </section>
  );
  ...
  return (
    <div style={{ marginBottom: 10 }}>
      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, marginBottom: 4 }}>
        <span style={{ color: "#8a9ab2" }}>{label}</span>
        <span style={{ color: "#e6ecf5", fontWeight: 600 }}>{fmtMoney(amount)}</span>
      </div>
      <div style={{ height: 6, background: "#0a1020", borderRadius: 999, overflow: "hidden" }}>
